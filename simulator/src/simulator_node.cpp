...
main( int argc, char* argv[] ){
	...
	ros::Subscriber landmarks_subscriber = node_handle.subscribe( "landmarks", 1, &Simulator::handle_landmarks, &simulator );
	ros::Subscriber obstacles_subscriber = node_handle.subscribe( "obstacles", 1, &Simulator::handle_obstacles, &simulator );
	...
	while( ros::ok() ){
		...
		simulator.update_observed_landmarks_and_observations();
