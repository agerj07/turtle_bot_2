...
handleRGBImage( const sensor_msgs::Image::ConstPtr& msg ){
	...
	for( unsigned int i = 0; i < detections.size(); i++ ){
		...
		detections[ i ].getRelativeTranslationRotation( tagsize, 525.0, 525.0,
			319.5, 239.5, translation, rotation );
		...
		observations.observations.back().range = sqrt( translation( 0 ) *
			translation( 0 ) + translation( 1 ) * translation( 1 ) );
		observations.observations.back().bearing = atan2( translation( 1 ),
			translation( 0 ) );
		observations.observations.back().signature = detections[ i ].id;
		cout << " (range,bearing,signature) = (" << observations.observations.
			back().range << "," << observations.observations.back().bearing << ","
